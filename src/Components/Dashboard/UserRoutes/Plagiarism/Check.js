...
function Check() {
  ...
  let url = "http://localhost:5000/api/proposal/draft/" + id;
  ...
  const handleSubmit = async () => {
    ...
    if (data2) {
      ...
      if (data2.google_similarity_score > 0.3) {
        setReject(true);
      } else {
        setLvl1(true);
      }
    ...
    // await alert("Level 1 similarity score is ",data.google_similarity_score)
  };

  const redirectHandler = () => {
    navigate(`/dashboard/check/${id}/view-plag`);
  };
  return (
    <>
      <Flex align="center" justify="center" direction="column" gap={8} p={2}>
        ...
        >
          ...
          {/* <Button onClick={redirectHandler}>View Plag</Button> */}
          <Text>{data1?.google_similarity_score} </Text>
          {data1?.urlresponse_list?.map(list => (<p><a href={list?.url}>{list?.url}</a> score:{ list?.similarity}</p>))}
      ...
      <div>Google ans : {data1?.google_similarity_score}</div>
    </>
  );
}
